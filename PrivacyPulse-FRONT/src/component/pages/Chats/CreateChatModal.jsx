...
const CreateChatModal = ({ isOpen, onClose, reFetchChats }) => {
	...
	const addChat = (id) => {
		...
		authFetch(`chats/create?id=${id}`, { method: "POST" }).then((r) => {
			if (r.ok) {
				enqueueSnackbar("Chat added successfully", { variant: "success" });
			} else {
				enqueueSnackbar("Error adding chat", { variant: "error" });
			}
		...
		<Modal open={isOpen} onClose={onClose} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
			...
			>
				<Box display={"flex"} width={"100%"} justifyContent={"space-between"} alignItems={"center"}>
					<Typography variant="h3">Add chat</Typography>
					...
					<Box display="flex" flexWrap="wrap" gap={1}>
						{users ? (
							users.map((user) => (
								...
								>
									<CardActionArea disabled={disabled} onClick={() => addChat(user.id)}>
